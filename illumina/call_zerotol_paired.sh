...
for gzfile in $@; do
	...
	cutadapt -a TGCGAGTAGTCGTTTGCGC  -o ${fileid}_cutadapt_R1.fastq.gz  $r1_gzfile  >> $outfile
	[[ $? == 0 ]] || { echo "An error occured in cutadapt R1 for ${fileid}" >> $outfile; continue; }
	cutadapt -a AACTATTTGTACAGCGACGC -o ${fileid}_cutadapt_R2.fastq.gz  $r2_gzfile  >> $outfile
